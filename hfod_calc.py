...
def process():
    # for now this is just based on daily run; later should have mid-day modes too (based on things that changed throughout the day)
...
def test_load_adhoc_report():
    ...
    with updated_settings({'adhoc_report_config':{
        ...
        assert [c for c in cells if c['l']=='cost_num' and c['c']=='test_1' and c['p']=='2017: q1' and c['r']=='WWR'][0]['e']==False
...
class Func_esum(Func):
    def __call__(self, adders=None, subtractors=None):
        """
        TODO improve docs: adds up line_items.  if all the things are null, returns null; if any things are there; returns the things
        """
        if adders:
