...
def preview_expression():
    ...
    dynamic_obj = settings['dynamic_obj']
    ...
    for s in sjclient.scroll(q, fields=['year', 'period'], sort='year desc,period desc'):
        ...
        by_region = rc.simulate_dynamic_obj_for_ui(dynamic_obj)
        results.append({
            'year': s['fields']['year'],
...
def load_adhoc_report():
    ...
    for c in adhoc_report_config['companies']:
        for p in adhoc_report_config['periods_obj']:
            report = get_report(c, p['year'], p['period'], 'usd')
            for r in adhoc_report_config['regions']:
                for l in adhoc_report_config['lineitems']:
                    val = report.get(r2f(r), {}).get(l)
                    if val is not None:
                        cells.append({
                            'c':c,
                            'p':'{}: {}'.format(p['year'], p['period']),
                            'r':r,
                            'l':l,
                            'v':val,
                        })
...
def test_load_adhoc_report():
    with updated_settings({'adhoc_report_config':{
        ...
        'lineitems': ['revenue_num','cost_num'],
        'regions':['US','WWR'],
        ...
        assert len(load_adhoc_report()['cells'])==6
...
class ReportCalc:
    def __init__(self, company, year, period, currency):
    ...
    def simulate_dynamic_obj(self, dynamic_obj):
        ...
        return {region_obj: by_li['_'].value for region_obj, by_li in self.calc_report(simulate_dynamic_obj=dynamic_obj).items() if by_li['_'].value}
    
    def simulate_dynamic_obj_for_ui(self, dynamic_obj):
        "Returns value for each region in *for_ui* format."
        return {region_obj: by_li['_'].for_ui for region_obj, by_li in self.calc_report(simulate_dynamic_obj=dynamic_obj).items()}

from collections import defaultdict
@sjutils.memoize
def get_report(company, year, period, currency='usd'):
    #Currently returns {'region_obj':{'li_num':12.34}} but TODO should change to v_obj structure (array of {'v*','l','r'})
...
def drop_non_leaf_fields(report):
    ...
    Ex. if both us_obj and noram_obj have a value for revenue_num, then noram_obj shoudln't have it any more after this function returns    
    ...
    for line_item in unique_line_items:
        ...
        for region_name, descendants in region_names_with_descendants.items():
            ...
            #we know which regions this line item has values for... if the current region doesn't have any descendants with a value, then we're a leaf!
...
def get_dynamic_objs(period):
    ...
    for series in sjclient.scroll('sid:HFOD\\LineItems set=dynamic_obj ', fields=['dynamic_obj']):
        ...
        dynamic_obj = series['fields']['dynamic_obj']
        dynamic_obj['field'] = fieldname 
...
def test_get_report():
    assert get_report('test_1', '2017', 'yr')['us_obj']['revenue_num'] == 100.0, 'Query "sid:HFOD\Reports company="test_1" year=2017 period=yr currency=usd" should return us_obj.revenue_num'
    
def test_trail():
    rc = ReportCalc('test_1', 2017, 'yr', 'usd')
    assert rc.simulate_dynamic_obj({
        'expr':"=TRAIL('revenue_num', 2)"
    }) == {'us_obj': 281.0, 'wwr_obj': 150.0, 'can_obj': 150.0, 'con_obj': 150.0, 'regwest_obj': 150.0, 'noram_obj': 150.0}
...
def test_get_full_report_with_rollups_and_calc():
    with updated_settings({'company': 'test_1', 'year': '2017', 'period': 'yr'}):
        r = get_full_report()
        assert r['noram_obj']['revenue_num']['v'] == 150.0, 'Should rollup US and Canada for sid=HFOD\Reports\test_1\2017\yr\usd'
...
def test_f():
    ...
    assert rc.simulate_dynamic_obj({
        'expr':"=F('revenue_num')"
    }) ==  {'us_obj': 100.0, 'wwr_obj': 150.0, 'can_obj': 50.0, 'con_obj': 150.0, 'regwest_obj': 150.0, 'noram_obj': 150.0}
    assert rc.simulate_dynamic_obj({
...
def XXX_test_preview_expression():
    with updated_settings({"dynamic_obj":{"expr":"=RO_CF_GROWTH_NUM*2","g_expr":""},"visibility_status":"Visible To Clients",
        "region":"non_geog","company":"AE","periodicity":"yr","notes":"","dynamic_req_dependencies":None}):
